...
const ProductCard: React.FC<Props> = ({ product, inCartQty, onAdd }) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`relative group flex flex-col bg-layer2 rounded-xl border transition-all duration-200 text-left overflow-hidden
        ${disabled
          ? 'opacity-50 cursor-not-allowed border-gray-100'
          : 'border-gray-100 hover:shadow-md hover:-translate-y-0.5 cursor-pointer active:scale-[0.98]'
        }`}
    >
      <div className="w-full h-28 bg-layer3 p-2 flex group-hover:scale-104 duration-300 items-center justify-center overflow-hidden">
        {product.image_url ? (
